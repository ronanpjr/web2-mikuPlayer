                        ...
                        <AnimatePresence>
                            {showPlaylist && (
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-3 md:grid-cols-4 gap-y-2 gap-x-2 pb-4">
                        ...
                        <AnimatePresence>
                            {showLyrics && (
                                ...
                                >
                                    ...
                                    <div className="player-card-glass rounded-3xl p-6 h-[400px] overflow-hidden relative">
                                        <Lyrics
                    ...
                    {showCreatePlaylistModal && (
                        ...
                        >
                            <motion.div
                                ...
                                className="player-card-glass w-full max-w-md border border-white/10 rounded-3xl shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]"
                ...
                <AnimatePresence>
                    {showSettings && (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md overflow-hidden"
                                ...
                                <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
                                        ...
                                        <div className="space-y-3 pt-2">
                                            ...
                                            <div className="grid grid-cols-6 gap-2">
                                                <button
                                                    ...
                                                    className={`h-10 rounded-xl flex items-center justify-center transition-all hover:scale-105 ${themeMode === 'auto' ? 'bg-white/20 ring-2 ring-white/20' : 'bg-white/5 hover:bg-white/10'}`}
                                                    title="Automático (Capa do Álbum)"
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" /><path d="m19 12-7 7-7-7" /><path d="M12 19V5" /></svg>
                                                </button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '0 221 192', secondary: '60 235 210', triangles: ['#00DDC0', '#3bebd4', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#00DDC0] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '0 221 192' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Miku"></button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '246 65 180', secondary: '249 130 200', triangles: ['#F641B4', '#f982c8', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#F641B4] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '246 65 180' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Luka"></button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '241 196 15', secondary: '247 220 100', triangles: ['#F1C40F', '#f7dc6f', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#F1C40F] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '241 196 15' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Rin/Len"></button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '69 179 157', secondary: '100 200 180', triangles: ['#45B39D', '#64c7b3', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#45B39D] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '69 179 157' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Gumi"></button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '231 76 60', secondary: '240 120 100', triangles: ['#E74C3C', '#f0786c', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#E74C3C] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '231 76 60' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Teto"></button>
                                                <button onClick={() => { setThemeMode('manual'); setManualTheme({ primary: '52 152 219', secondary: '100 180 240', triangles: ['#3498DB', '#64b4f0', '#ffffff'] }); }} className={`h-10 rounded-xl bg-[#3498DB] transition-all hover:scale-105 ${themeMode === 'manual' && manualTheme?.primary === '52 152 219' ? 'ring-2 ring-white ring-offset-2 ring-offset-zinc-900' : ''}`} title="Kaito"></button>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <section>
                                        ...
                                        <div className="bg-black/20 rounded-2xl p-6 border border-white/5">
                                            <div className="flex justify-between h-40 gap-2 overflow-visible py-4">
                                                {['60', '250', '1k', '4k', '12k'].map((label, idx) => (
                                                    <div key={idx} className="flex flex-col items-center gap-2 group w-full h-full relative">
                                                        <div className="relative w-2 flex-1 bg-white/5 rounded-full group-hover:bg-white/10 transition-colors my-2">
                                                            {/* Bar */}
                                                            <div className="absolute bottom-0 left-0 w-full bg-primary/30 h-1/2 rounded-b-full"></div>
                                                            {/* Active Level */}
                                                            <div className="relative w-full h-full flex items-end justify-center rounded-full overflow-hidden">
                                                                <input
                                                                    type="range"
                                                                    min="-12"
                                                                    max="12"
                                                                    ...
                                                                    className="absolute top-1/2 left-1/2 w-[160px] h-8 opacity-0 cursor-pointer z-10"
                                                                    title={`${label}Hz: ${eqBands[idx]}dB`}
                                                                    style={{
                                                            ...
                                                            {/* Thumb Indicator */}
                                                            <div
                                                                className="absolute w-6 h-6 bg-white rounded-full shadow-lg border-2 border-primary pointer-events-none transition-all duration-100 ease-out flex items-center justify-center translate-y-1/2 left-1/2 -translate-x-1/2"
                                                                style={{ bottom: `${((eqBands[idx] + 12) / 24) * 100}%` }}
                                                            >
                                            ...
                                            <div className="grid grid-cols-2 sm:flex gap-2 justify-center pt-4">
                                                {[
